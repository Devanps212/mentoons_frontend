...
const Details = () => {
  return (
    <ul className="flex flex-col justify-center items-center gap-3 pt-10 px-0 md:px-2 lg:px-4">
      {COMMUNITY.map(({ title, description, color, link }, index) => (
        <Link to={link}>
          <li
            key={index}
            className="group flex flex-row justify-between items-center gap-5 fredoka w-full max-w-4xl p-5 rounded-lg transition-all hover:shadow-lg"
          >
            <div
